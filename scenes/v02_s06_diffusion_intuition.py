...
class V02DiffusionIntuition(TextPixelsScene):
    ...
    def timeline_highlight_diffusion(self):
        ...
        items = [
            ("2015", "U-Net",        IMAGE_BLUE),
            ("2015", "Diffusion",    FORWARD_COLOR),
            ("2017", "Transformers", TEXT_PURPLE),
            ("2020", "CLIP",         TEXT_PURPLE),
            ("2021", "VQGAN",        PUZZLE_GOLD),
            ("2021", "DALL-E",       PUZZLE_GOLD),
            ("2021", "LDM",          POSITIVE_GREEN),
        ]
        pieces = VGroup()
        for year, name, color in items:
            piece = self.puzzle_piece(year, name, color)
            pieces.add(piece)
        pieces.arrange(RIGHT, buff=0.14).scale(0.90).move_to([0, -0.1, 0])
        timeline = Line(
            pieces.get_left() + 0.22 * LEFT + 0.82 * DOWN,
            pieces.get_right() + 0.22 * RIGHT + 0.82 * DOWN,
            color=DIM, stroke_width=2,
        )
        ticks = VGroup(*[
            Line(p.get_bottom(), p.get_bottom() + 0.5 * DOWN,
                 color=p[0].get_stroke_color(), stroke_width=1.4)
            for p in pieces
        ])
        diffusion_piece = pieces[1]  # index 1 = "Diffusion"
        glow = SurroundingRectangle(diffusion_piece, color=FORWARD_COLOR, buff=0.1, stroke_width=3)

        self.play(FadeIn(tag), FadeIn(title, shift=0.1 * DOWN), Create(timeline), run_time=1.0)
        start_pos = [p.get_center() + np.array([0, 2.6 + 0.2 * (i % 2), 0]) for i, p in enumerate(pieces)]
        for piece, pos in zip(pieces, start_pos):
            piece.move_to(pos)
        target_pos = pieces.copy().arrange(RIGHT, buff=0.14).scale(0.90).move_to([0, -0.1, 0])
        self.play(
            LaggedStart(*[p.animate.move_to(target_pos[i].get_center()) for i, p in enumerate(pieces)], lag_ratio=0.12),
            run_time=2.0,
        )
        self.play(LaggedStart(*[Create(t) for t in ticks], lag_ratio=0.06), run_time=0.6)
        self.play(Create(glow), Flash(diffusion_piece, color=FORWARD_COLOR), run_time=1.0)
        ...
        self.play(FadeIn(sub, shift=0.1 * UP), run_time=0.8)
        self.wait(8.0)
