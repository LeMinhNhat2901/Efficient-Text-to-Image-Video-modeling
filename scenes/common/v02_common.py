...
class TextPixelsScene(DiffusionScene):
    ...
    def voiceover_duration(self, path: Path) -> float | None:
        """Return WAV duration when narration exists, without depending on ffmpeg."""
        if not path.exists():
            return None
        try:
            import wave
    ...
    def hold_for_voiceover(self, start_time: float, path: Path, pad: float = 0.0) -> None:
        duration = self.voiceover_duration(path)
        if duration is not None:
            self.hold_to_time(start_time, duration + pad)
