...
class V02DiffusionMath(TextPixelsScene):
    ...
    def simple_loss(self):
        ...
        def readable_chip(text: str, tex: str, color: str, width: float, height: float = 1.02) -> VGroup:
            box = self.soft_box(width, height, color=color, fill_opacity=0.065, stroke_opacity=0.72)
            text_lbl = self.label(text, 13, MUTED, font=FONT_BODY)
            math_lbl = self.math_label(tex, 36, color)
            self.fit_to_box(math_lbl, width - 0.28, 0.50)
            content = VGroup(text_lbl, math_lbl).arrange(DOWN, buff=0.02).move_to(box)
            return VGroup(box, content)

        # Model diagram
        model_box = readable_chip("model output", r"\varepsilon_\theta(x_t,t)", TEXT_PURPLE, 3.35, 1.08).move_to([0, 0.8, 0])
        input_chip1 = readable_chip("noisy image", r"x_t", NOISE_GRAY, 2.35).move_to([-3.8, 1.4, 0])
        input_chip2 = readable_chip("timestep", r"t", PUZZLE_GOLD, 2.0).move_to([-3.8, 0.2, 0])
        target_chip = readable_chip("true noise", r"\varepsilon", PUZZLE_GOLD, 2.05).move_to([3.8, 0.8, 0])
        ...
        try:
            loss_eq = self.display_equation(
                ...
                width=9.4, size=39, accent=TEXT_PURPLE,
            ).move_to([0, -1.4, 0])
        except Exception:
            loss_eq = self.label(
                "L_simple  =  E[ ||eps − eps_theta(x_t, t)||^2 ]",
                26, TEXT_PURPLE, font=FONT_CODE,
            ).move_to([0, -1.4, 0])

        # Color annotations
        eps_note_math = self.math_label(r"\varepsilon", 30, PUZZLE_GOLD)
        eps_note_text = self.label("true noise", SMALL_SIZE, TEXT, font=FONT_BODY)
        eps_note = VGroup(eps_note_math, eps_note_text).arrange(RIGHT, buff=0.16).move_to([-2.5, -2.8, 0])

        pred_note_math = self.math_label(r"\varepsilon_\theta", 30, REVERSE_CYAN)
        pred_note_text = self.label("predicted noise", SMALL_SIZE, TEXT, font=FONT_BODY)
        pred_note = VGroup(pred_note_math, pred_note_text).arrange(RIGHT, buff=0.16).move_to([2.5, -2.8, 0])
