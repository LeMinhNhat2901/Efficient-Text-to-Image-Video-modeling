...
class V02DiscussionFinale(TextPixelsScene):
    ...
    def closing_montage(self):
        ...
        landing = pieces.copy()
        landing.arrange_in_grid(rows=2, cols=6, buff=(0.16, 0.18))
        landing.scale_to_fit_width(10.65).move_to([0, -0.72, 0])
        self.play(
            LaggedStart(
                *[p.animate.move_to(landing[i].get_center()).scale(0.86) for i, p in enumerate(pieces)],
                lag_ratio=0.04,
            ),
            run_time=1.6,
        )
        ...
        self.wait(12.0)
        self.play(FadeOut(pieces), FadeOut(road), FadeOut(doors), FadeOut(final_lines), FadeIn(thanks), run_time=1.2)
        self.wait(38.0)
